...
public class BcpInsertMultiTask implements Callable<Long>{
	
	private final static Logger logger = Logger.getLogger(BcpInsertMultiTask.class);
    ...
    private List<Future<Long>> batchReply = new ArrayList<Future<Long>>();
	 ...
	 BcpInsertMultiTask(BcpData bcpData,BlockingQueue<BcpData> doneQueue,DataSource dataSource, int batchInsert, BcpConfig bcpConfig,
			...
			while ((bcpData = csvDataQueue.take()) != null) {
				     ...
				     if ((ctr) % batchInsert == 0) {
				  	     logger.debug("DB executing partial update>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
					 		   Callable<Long> batchTask = new BatchExecute( conn,bcpData,bcpConfig);
	...
	class BatchExecute implements Callable<Long> {
		   ...
		   public BatchExecute(Connection conn, BcpData bcpData, BcpConfig bcpConfig ){
				  try {
					this.prepStateMent = conn.prepareStatement( bcpConfig.getBcpInsertStmnt() );
					this.bcpConfig = bcpConfig;
