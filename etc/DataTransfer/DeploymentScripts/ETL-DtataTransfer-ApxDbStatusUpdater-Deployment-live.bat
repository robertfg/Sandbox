...
::Deployment of ETL-DtataTransfer - ApxDbStatusUpdater-live

::ETL service names
set serviceName1=ApxDbStatusUpdater

::ETL app name - make sure it is the correct version 
set etlAppName=mx

:: tomcat server locations
set tomcatLocation1=F:\ETL\ApxDbStatusUpdater\prelive


::Get date as yyyymmdd_hhmm 
set date=%date:~-4%%date:~7,2%%date:~4,2%_%time:~0,2%%time:~3,2%

echo ********************ApxDbStatusUpdater-live********************
...
echo ETL-DtataTransfer - ApxDbStatusUpdater live
pause
...
md .\Rollback\%date%
ping -n 10 127.0.0.1 > NUL
echo %etlAppName% and %etlAppName%.war will be backed up here .\Rollback\%date%\

pause 

move %tomcatLocation1%\webapps\%etlAppName%.war .\Rollback\%date%\
XCOPY %tomcatLocation1%\webapps\%etlAppName% .\Rollback\%date%\%etlAppName%\ /D /E /C /R /K /Y /I
...
rd /q/s %tomcatLocation1%\webapps\%etlAppName% 

ping -n 10 127.0.0.1 > NUL

echo Step 2 - Back up the older version here - .\Rollback\%date%

copy %etlAppName%.war %tomcatLocation1%\webapps\

echo Step 3 - New .war version of %etlAppName% is copied to %tomcatLocation1%\webapps\
