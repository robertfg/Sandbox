 ...
 *
 */
public class TransposeDataHelperSR 
{
	...
	public TransposeDataHelperSR(TransposeRequestInput transposeRequest)
	{
		this.transposeRequest=transposeRequest;		
	}
	
	
	public DrillthroughResultDTO transpose(DrillthroughResultDTO result, List<String> scnTerm ) 
	{
		StopWatch stopWatch = new StopWatch();	
		Set<String> tenors=new TreeSet<String>(new TenorComparator());	
		...
		for (Entry<Point, DrillThroughRowContent> entry:transposeRequest.getTransposedData().entrySet())
		{
			Object[] rawData=entry.getValue().getContent();
				List<Object> newData=populateNewRow(tenors, skipColsIndex, entry.getValue(), rawData,scnTerm);
	...
	{
		return !StringUtils.hasText(spreadType) || "N/A".equalsIgnoreCase(spreadType);		
	}	
	 ...
	 */
	private List<Object> populateNewRow(Set<String> tenors, Set<Integer> skipColsIndex,DrillThroughRowContent rowContent, Object[] rawData,List<String> scnTerm) 
	{
		Map<Headers,Integer> headerIdx=transposeRequest.getColumnDetails().getHeaderIdx();
		List<Object> newData=new ArrayList<Object>();
			...
			for(Object tenor:tenors){
				if( scnTerm!=null && scnTerm.contains( tenor ) ){
