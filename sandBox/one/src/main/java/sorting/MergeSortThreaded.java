...
public class MergeSortThreaded {
	...
	public static void main(String[] args) throws Throwable {
		int LENGTH = 1;   // initial length of array to sort
	...
	public static void parallelMergeSort(int[] a, int threadCount) {
		if (threadCount <= 1) {
			...
			// mergeSort(left); 
			// mergeSort(right);
			Thread lThread = new Thread(new MergeSorterThread(left,  threadCount / 2));
			Thread rThread = new Thread(new MergeSorterThread(right, threadCount / 2));
			lThread.start();
			rThread.start();
