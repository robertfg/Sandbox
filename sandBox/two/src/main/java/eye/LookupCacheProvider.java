...
public class LookupCacheProvider implements LookupCache {
				  ...
				  if (lookUpUtil.containsKeyValue(paramValueIndex, paramKeyCode, STR_BLANK_CODE  )) { //search in paramValue
					  ...
					  int[] pValueIndex  = paramValueIndex.getIndex(paramKeyCode, STR_BLANK_CODE) ;
					  		  ...
					  		  for (int i = 0; i < pValueIndex.length; i++) {
							    	 System.out.println("ss:"+ new String(strPool.getValue( paramValueIndex.getValue(pValueIndex[i]) )) );
							    	 int[] segValueIndex = segmentValueIndex.getIndex(paramKeyCode,  paramValueIndex.getValue(pValueIndex[i]) );
	...
	public static void main(String[] args) {
		...
		String json = "/home/degs/devs/space/dilzio-eyeota-coding-project-c400cb339a60/src/test/resources/data2.json";
			json = "c:\\devs\\sample.json";
		
		LookupCacheProvider provider = new LookupCacheProvider();
		provider.test(json);
